...
def draw_robot_path(robot, graph, color=(255, 255, 0)):
    if robot.path:
...
while running:
    ...
    screen.fill(WHITE)
    draw_nodes(graph)  # Draw all nodes
    draw_room(living_room)  # Draw rooms
    draw_room(kitchen)
    draw_robot_path(robot, graph)  # Draw the robot's path in yellow
    draw_robot(robot)  # Draw the robot itself
